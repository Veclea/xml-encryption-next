...
const keyInfoTemplate = ({ encryptionPublicCert, encryptedKey, keyEncryptionMethod, keyEncryptionDigest, keyEncryptionMgf1 }) => {
    // 如果有 mgfAlgorithm，说明是 XML Enc 1.1 格式
    const mgfBlock = keyEncryptionMgf1
        ? `<MGF Algorithm="${escapeHtml(keyEncryptionMgf1)}"></MGF>`
        : '';
