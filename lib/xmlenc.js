...
function encryptKeyInfoWithSchemeInternal({ symmetricKey, options, padding }) {
    ...
    if (options.keyEncryptionAlgorithm === 'http://www.w3.org/2001/04/xmlenc#rsa-1_5') {
        encrypted = publicKey.encrypt(
            Buffer.from(symmetricKeyBuffer, 'binary').toString('base64'),
            'RSAES-PKCS1-V1_5'
        );
    ...
    if (options.keyEncryptionAlgorithm === 'http://www.w3.org/2009/xmlenc11#rsa-oaep') {
        ...
        // 单独处理 MGF1 哈希
        try {
            mgf1 = createForgeMD(options.keyEncryptionMgf1 || 'sha1');
        } catch (e) {
    ...
    const execute = () => {
        switch (options.keyEncryptionAlgorithm) {
            ...
            case 'http://www.w3.org/2009/xmlenc11#rsa-oaep':
                options.keyEncryptionDigest = options.keyEncryptionDigest || 'sha256';
                options.keyEncryptionMgf1 = options.keyEncryptionMgf1 || 'sha256';
                return encryptKeyInfoWithSchemeInternal({
                    symmetricKey, options, padding: crypto.constants.RSA_PKCS1_OAEP_PADDING
                });
...
function encryptInternal(content, options) {
    ...
    const keyInfo = (() => {
        ...
        switch (options.keyEncryptionAlgorithm) {
            ...
            case 'http://www.w3.org/2001/04/xmlenc#rsa-oaep-mgf1p':
                encryptKeyInfoOptions.keyEncryptionDigest = encryptKeyInfoOptions.keyEncryptionDigest || 'sha256';
            ...
            case 'http://www.w3.org/2009/xmlenc11#rsa-oaep':
                encryptKeyInfoOptions.keyEncryptionDigest = encryptKeyInfoOptions.keyEncryptionDigest || 'sha256';
                encryptKeyInfoOptions.keyEncryptionMgf1 = encryptKeyInfoOptions.keyEncryptionMgf1 || 'sha256';
                return encryptKeyInfoWithSchemeInternal({
                    symmetricKey, options: encryptKeyInfoOptions, padding: crypto.constants.RSA_PKCS1_OAEP_PADDING
                });
            default:
...
function decryptKeyInfo(doc, options) {
    ...
    let keyEncryptionMethod = xpath.select(
        "//*[local-name(.)='KeyInfo']/*[local-name(.)='EncryptedKey']/*[local-name(.)='EncryptionMethod']",
        doc
    )[0];

    if (!keyEncryptionMethod) {
        const keyRetrievalMethod = xpath.select(
            "//*[local-name(.)='EncryptedData']/*[local-name(.)='KeyInfo']/*[local-name(.)='RetrievalMethod']",
            doc
        )[0];
        keyRetrievalMethodUri = keyRetrievalMethod ? keyRetrievalMethod.getAttribute('URI') : null;
        keyEncryptionMethod = keyRetrievalMethodUri
            ? xpath.select(
                "//*[local-name(.)='EncryptedKey' and @Id='" + keyRetrievalMethodUri.substring(1) + "']/*[local-name(.)='EncryptionMethod']",
                doc
              )[0]
            : null;
    }
    ...
    const mgfDigestMethod = xpath.select(
        "//*[local-name(.)='KeyInfo']/*[local-name(.)='EncryptedKey']/*[local-name(.)='EncryptionMethod']/*[local-name(.)='MGF']",
        doc
    )[0];
    ...
    const encryptedKeyNode = keyRetrievalMethodUri
        ? xpath.select(
            "//*[local-name(.)='EncryptedKey' and @Id='" + keyRetrievalMethodUri.substring(1) + "']/*[local-name(.)='CipherData']/*[local-name(.)='CipherValue']",
            keyInfo
          )[0]
 ...
 * @returns {Buffer} 解密后的对称密钥
 */
function decryptKeyInfoWithSchemeInternal({ keyEncryptionAlgorithm, encryptedKeyNode, options, oaepHash, mgfHash }) {
    // 安全检查
    ...
    try {
        ...
        if (keyEncryptionAlgorithm === 'http://www.w3.org/2001/04/xmlenc#rsa-1_5') {
            ...
            const mgf1 = mgfHash ? createForgeMD(mgfHash) : forge.md.sha1.create();
