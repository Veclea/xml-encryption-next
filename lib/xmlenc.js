...
function encryptKeyInfoWithScheme({symmetricKey, options, padding, callback}) {
    ...
    // Check if insecure hash algorithms are disallowed and check for them
    if (options.disallowInsecureHash &&
        options.keyEncryptionDigest &&
        options.keyEncryptionDigest === 'sha1') {
        return callback(new Error('SHA-1 hash algorithm is not secure and has been disabled'));
    }
...
function encrypt(content, options, callback) {
    ...
    // Check for insecure encryption algorithms if flag is set
    if (options.disallowInsecureEncryption &&
        ...
        return callback(new Error('AES-CBC encryption algorithm is not secure and has been disabled'));
    }
