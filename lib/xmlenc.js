...
function encryptKeyInfoWithScheme(symmetricKey, options,padding,callback) {
    ...
    try {
        let base64EncodedEncryptedKey;

        if (padding === crypto.constants.RSA_PKCS1_PADDING) {
            // Fallback to crypto for RSA-1_5
                let encrypted = crypto.publicEncrypt({
                    key: options.rsa_pub,
                    padding: padding
                }, symmetricKeyBuffer);
            ...
            // Use node-forge for OAEP with custom MGF
            const publicKey = forge.pki.publicKeyFromPem(options.rsa_pub.toString());
            const keyBytes = new Uint8Array(symmetricKeyBuffer);
            if(options.keyEncryptionAlgorithm === 'http://www.w3.org/2001/04/xmlenc#rsa-oaep-mgf1p'){
              console.log("走的是这里====================")
                console.log(options.keyEncryptionDigest)
              console.log("mgf1p加密走这里====")
                let md;
                ...
                switch (options.keyEncryptionDigest) {
                    ...
                    default:
                        md = forge.md.sha56.create(); break;
                        break;
                }
                const encryptedBytes = publicKey.encrypt(keyBytes, 'RSA-OAEP', {
                    md: md,
                    mgf1: {
                        md: mgf1
                    }
                ...
                base64EncodedEncryptedKey = forge.util.encode64(encryptedBytes);
                let params = {
                    encryptedKey: base64EncodedEncryptedKey,
                    encryptionPublicCert: '<X509Data><X509Certificate>' + pemToCert(options.pem.toString()) + '</X509Certificate></X509Data>',
                    keyEncryptionMethod: options.keyEncryptionAlgorithm,
                    keyEncryptionDigest: hashObjet['keyEncryptionDigest'][options.keyEncryptionDigest]
                };
                ...
                let md, mgf1;
                switch (options.keyEncryptionDigest || 'sha1') {
                    case 'sha1': md = forge.md.sha1.create(); break;
                    /*                case 'sha256': md = forge.md.sha256.create(); break;*/ //node-forge not supported
                    case 'sha224': md = forge.md.sha224.create(); break;
                    case 'sha384': md = forge.md.sha384.create(); break;
                    case 'sha512': md = forge.md.sha512.create(); break;
                    default: return callback(new Error('Unsupported OAEP hash: ' + options.oaepHash));
                }

                switch (options.keyEncryptionMgfAlgorithm || 'sha1') {
                    case 'sha1': mgf1 = forge.md.sha1.create(); break;
                    /*    case 'sha224': mgf1 = forge.md.sha224.create(); break;*/
                    case 'sha256': mgf1 = forge.md.sha256.create(); break;
                    case 'sha384': mgf1 = forge.md.sha384.create(); break;
                    case 'sha512': mgf1 = forge.md.sha512.create(); break;
                    default: return callback(new Error('Unsupported MGF hash: ' + options.mgfHash));
                }
                ...
                let params = {
                    ...
                    encryptionPublicCert: '<X509Data><X509Certificate>' + pemToCert(options.pem.toString()) + '</X509Certificate></X509Data>',
                    keyEncryptionMethod: options.keyEncryptionAlgorithm,
                    keyEncryptionDigest: options.keyEncryptionDigest ? hashObjet['keyEncryptionDigest'][options.keyEncryptionDigest]:hashObjet['keyEncryptionDigest']['sha1'],
                    mgfAlgorithm:options.keyEncryptionMgfAlgorithm? hashObjet['mgfAlgorithm'][options.keyEncryptionMgfAlgorithm]: hashObjet['mgfAlgorithm']['sha1']// Pass to template
                };
...
function encryptKeyInfo(symmetricKey, options, callback) {
    ...
    options.keyEncryptionDigest = options.keyEncryptionDigest || 'sha256';
    options.keyEncryptionMgfAlgorithm = options.keyEncryptionMgfAlgorithm || 'sha256';
...
function decryptKeyInfo(doc, options) {
    ...
    if (keyDigestMethod) {
        ...
        if(keyDigestMethodAlgorithm?.includes('sha224')){
            throw new Error('current keyDigestMethod is not supported');
        }
    ...
    if (MgfDigestMethod) {
        ...
        if(MgfDigestMethodAlgorithm === 'http://www.w3.org/2009/xmlenc11#mgf1sha224'){
            throw new Error('current mgf1 is not supported');
        }
    ...
    switch (keyEncryptionAlgorithm) {
        case 'http://www.w3.org/2009/xmlenc11#rsa-oaep':
            return decryptKeyInfoWithScheme(encryptedKey, options, crypto.constants.RSA_PKCS1_OAEP_PADDING, oaepHash,mgfHash,keyEncryptionAlgorithm);
        case 'http://www.w3.org/2001/04/xmlenc#rsa-oaep-mgf1p':
            return decryptKeyInfoWithScheme(encryptedKey, options, crypto.constants.RSA_PKCS1_OAEP_PADDING, oaepHash,mgfHash,keyEncryptionAlgorithm);
        case 'http://www.w3.org/2001/04/xmlenc#rsa-1_5':
...
function decryptKeyInfoWithScheme(encryptedKey, options, padding, oaepHash,mgfHash,keyEncryptionAlgorithm) {
  console.log(oaepHash,mgfHash)
  console.log("解密的时候哟")
    ...
    try {
        ...
        if(keyEncryptionAlgorithm === 'http://www.w3.org/2009/xmlenc11#rsa-oaep'){
            ...
            if(mgfHash){
                ...
                console.log("走这里解密没问题啊2xmlenc11#rsa-oaep")
                const decrypted = privateKey.decrypt(encryptedKeyBytes, 'RSA-OAEP', {
                    md: md,  // 设置用于 OAEP 的哈希算法,
                    mgf1: {
                        md:md
